...
const calculatePassword = (inputArray: number[], part: 1 | 2): number => {
  ...
  for (const delta of inputArray) {
    ...
    if (part === 2) {
      ...
      const partialMove = oldLocation + (delta % 100)

      // If moving right, Math.sign(delta), the value must be greater than 100 to cross zero
      // If moving left, the value must be less than 0, when not starting at 0, to cross zero
      if ((Math.sign(delta) > 0 && partialMove > 100) || (oldLocation !== 0 && partialMove < 0)) {
        zeroCount++
      }
    }
